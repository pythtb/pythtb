...
class HRBlock:
    h: np.ndarray          # (num_wan, num_wan) complex
    degeneracy: int        # WS-cell degeneracy for this R
...
def parse_unit_cell_cart(win_lines: List[str]) -> np.ndarray:
    ...
    if block[0].lower() in {"bohr", "ang", "angstrom"}:
        if block[0].lower() == "bohr": scale = BOHRTOANG
        block = block[1:]
...
def _cart_to_red(a1,a2,a3, xyz):
    B = np.linalg.inv(np.vstack([a1,a2,a3]).T)  # columns are recip in 1/Ang if needed
...
def read_hr(root: Path, prefix: str) -> Tuple[int, Dict[Tuple[int,int,int], HRBlock]]:
    ...
    ham_r = {tuple(map(int, unique_R[k])): HRBlock(h=blocks[k], degeneracy=int(deg[k]))
...
def _format_k_label(label: str) -> str:
    special = {
        "g": r"\Gamma", "gamma": r"\Gamma", "Γ": r"\Gamma",
        "delta": r"\Delta", "Δ": r"\Delta", "theta": r"\Theta", "Θ": r"\Theta",
        "lambda": r"\Lambda", "λ": r"\Lambda", "xi": r"\Xi", "ξ": r"\Xi",
        "pi": r"\Pi", "π": r"\Pi", "sigma": r"\Sigma", "σ": r"\Sigma",
        "upsilon": r"\Upsilon", "υ": r"\Upsilon", "phi": r"\Phi", "ϕ": r"\Phi",
        "psi": r"\Psi", "ψ": r"\Psi", "omega": r"\Omega", "ω": r"\Omega",
    }
    raw = label.strip()
    if not raw: 
        return "$$"
    ...
    key = base.lower()
    latex = special.get(key) or (base if (len(base)==1 and base.isalpha()) else rf"\mathrm{{{base}}}")
...
def read_kpoint_path(win_lines: List[str], *, latex=True):
    ...
    for line in block:
        ...
        for o in range(0, len(toks), 4):
            lbl = toks[o]
            coord = np.array([float(toks[o+1]), float(toks[o+2]), float(toks[o+3])])
...
def load_w90_dataset(root: Path | str, prefix: str) -> W90Dataset:
    ...
    try:
        ...
        pass
    return W90Dataset(prefix=prefix, root=root, lat_cart=lat,
                      centres_xyz=centres_xyz, centres_red=centres_red,
                      num_wan=num_wan, ham_r=ham_r,
                      kpath_nodes_red=k_nodes, kpath_labels=k_labels,
                      bands_k_red=bands_k, bands_ene_ev=bands_ene,
                      meta={})

def read_bands_w90(root: Path | str, prefix: str, num_wan: int) -> Tuple[np.ndarray, np.ndarray]:
    """
