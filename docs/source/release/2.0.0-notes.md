...
PythTB 2.0.0 builds on over a decade of community work to make the package easier to extend, faster on large problems, and friendlier for teaching and research. This release reorganizes the codebase into a modular package, introduces vectorized workflows for substantial speedups, and adds I/O utilities that interoperate more naturally with first-principles tools—all while preserving the spirit and core capabilities that made PythTB widely used.
...
This document summarizes the design philiosophy, API changes, and migration guidance.

## Why PythTB 2.0.0?

The original package had a single-file (`pythtb.py`) design that served early use cases well. However, as the codebase expanded, this structure became unwieldy. Version 2.0.0 reorganizes the project into a modular structure with a more intutive API and consistent naming, following PEP-8 and scientific Python community best practices.
...
mesh = Mesh(dim_k=1, axis_types=['k'])
k_pts = np.linspace(0, 1, 100)
mesh.build_custom(k_pts)
mesh.wind_bz(0, 0)  # winds kx around the BZ
```
