        ...
        for line in f:
            ...
            if stripped.startswith("- **") and stripped.endswith("/**"):
                ...
                if match:
                    ...
                    stack[-1].setdefault("_tests", []).append(
                        (name, status == "✅", time)
                    )
    return tree
...
def merge_trees(existing, new):
    for key, value in new.items():
        if key == "_tests":
            existing_tests = {
                name: (status, ts) for name, status, ts in existing.get("_tests", [])
            }
            for name, status, timestamp in value:
                existing_tests[name] = (status, timestamp)
            existing["_tests"] = sorted(
                [(k, v[0], v[1]) for k, v in existing_tests.items()]
            )
        else:
