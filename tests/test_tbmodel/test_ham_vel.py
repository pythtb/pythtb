...
def test_hamiltonian_spinful(fkm_model):
    ...
    # check hermiticity
    np.testing.assert_allclose(H_nonflat, np.conj(np.transpose(H_nonflat, (0, 3, 4, 1, 2))))
...
def test_velocity_parametric_ssh(ssh_model):
   ...
   # create 1d model with orbs at origin, cart and non-cart vel should coincide
    ...
    with pytest.raises(AssertionError):
        np.testing.assert_allclose(vel_cart_shift, vel_frac_shift, rtol=1e-10, atol=1e-10)
