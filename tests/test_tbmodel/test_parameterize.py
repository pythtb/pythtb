...
def test_ribbon():
    ...
    np.testing.assert_array_equal(tb.lat_vecs, [[1, 0], [0, 2]], "lat_vecs should match")
    np.testing.assert_array_equal(tb.orb_vecs, [[0, 0], [0.5, 0.5], [0.5, 1.5]], "orbital positions should match")
    ...
    np.testing.assert_array_equal(tb.onsite, [0.0, 1.0, 0.5], "Onsite energies should match")
    ...
    assert tb.onsite[0] == 0, "Onsite energy for orbital 0 should be 0 as a placeholder for parameter"

    tb.set_parameters({"U": 2.5})
    assert tb.onsite[0] == 2.5, "Onsite energy for orbital 0 should be updated to 2.5"

    tb.set_hop("t1", 2, 0, [0, -1])  # hopping from orbital 2 to 0 shifted by one cell back along periodic axis
    assert len(tb.hoppings) == 2, "There should still be 2 hoppings defined after adding parameterized hopping"

    tb.set_parameters(t1 = 0.3)
    assert len(tb.hoppings) == 3, "There should now be 3 hoppings defined after setting parameters"
    assert tb.hoppings[2]["amplitude"] == 0.3, "Hopping from 2 to 0 should be updated to 0.3"
    ...
    evals, evecs = tb.solve_ham(k_pts=np.linspace(0,1, 10), return_eigvecs=True, U=0.4, t=0.7)
    ...
    assert evecs.shape == (10, 4, 4, 3, 3), "Eigenvectors shape should be (10, 4, 4, 3, 3)"
